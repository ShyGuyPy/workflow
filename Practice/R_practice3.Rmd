...
active_data.df <- data.table::fread(paste0(cedr_path, "cedr", "_raw", ".csv"),
                                    data.table = FALSE)
...
## generate unique_id
unique_id <- rownames(data_last.df)
data_last.df <- cbind(unique_id=unique_id, data_last.df)
...
for(i in 1:repeat_me) {
  ...
  temp.df[] <- lapply(data_last.df, jitter) #factor=1, amount = NULL))
  
  ####need to assign unique ids first----*
  test.df <- data_last.df %>% rename_at(vars(-unique_id), ~ paste0(., i))
  }
