...
repeat.scores.df <- test.df %>%              
 ...
 arrange(unique_id)
  ...
  for(i in 1:repeat_me) {
    ...
    # rs.df <- repeat.scores.df %>%
    # rename(count  = name)#as.character(paste0("Score", count))) 
    #print(as.character(paste0("Score", i)))
    
    # rs.df <- rs.df %>%
    #   mutate(i= i)
    
    #rs.df$test_column <- 12
    repeat.scores.df <- repeat.scores.df %>%
      mutate(test_column = i)
    
    #as.character(paste0("Score", i))
    
    colnames(repeat.scores.df)[colnames(repeat.scores.df)=="test_column"] <- as.character(paste0("Score", i))
